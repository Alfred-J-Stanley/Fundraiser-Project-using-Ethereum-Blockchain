...
contract("Fundraiser", (accounts) => {
  ...
  describe("making donations", () => {
    ...
    const donar = accounts[2];

    it("increases myDonationCount", async () => {
      const currentDonationsCount = await fundraiser.myDonationsCount({
        from: donar,
      });
      await fundraiser.donate({ from: donar, value });
      ...
      assert.equal(
        ...
        "myDonationsCount should be increase by 1"
      );
    });
    it("includes my donation in myDonations", async () => {
      await fundraiser.donate({ from: donar, value });
      const { values, dates } = await fundraiser.myDonations({ from: donar });

      assert.equal(value, values[0], "values should match");
      assert(dates[0], "dates should present");
    });
    ...
    it("increases the donationCount", async () => {
      ...
      assert.equal(
        1,
        newDonationsCount - currentDonationsCount,
        "donation count should increment by 1"
      );
    });
    it("emits the donationReceived event", async () => {
      const tx = await fundraiser.donate({ from: donar, value });
  ...
  describe("fallback function", () => {
    ...
    it("increases totalDonationsCount amount", async () => {
      const currentTotalDonations = await fundraiser.totalDonations();
    ...
    it("increases donation count", async () => {
      ...
      assert.equal(diff, 1, "donations count should increment by one");
    });
  });
});
