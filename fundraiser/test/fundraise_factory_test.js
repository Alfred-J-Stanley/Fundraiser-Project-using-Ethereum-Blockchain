...
contract("FundraiserFactory: createFundraiser", (accounts) => {
  ...
  it("increanents the fundraiserCount", async () => {
  ...
  it("emits the FundraiserCreate event", async () => {
    fundraiserFactory = await FundraiserFactoryContract.deployed();
...
contract("fundraiserFactory: fundraisers", (accounts) => {
  ...
  describe("variying limits", () => {
    ...
    it("returns 10 results when limit requested is 10 ", async () => {
      const fundraiser = await factory.fundraisers(10, 0);
      assert.equal(fundraiser.length, 10, "results size should be 10");
    });
    it("returns 20 results when limit requested is 20", async () => {
      const fundraiser = await factory.fundraisers(20, 0);
      assert.equal(fundraiser.length, 20, "results size should be 20");
    });
    it("returns 20 results when limit requested is 30", async () => {
      const fundraiser = await factory.fundraisers(30, 0);
      assert.equal(fundraiser.length, 20, "results size should be 20");
    });
  });

  describe("variying offset", () => {
    let factory;
    beforeEach(async () => {
    ...
    it("contains the fundraiser with the appropriate offset", async () => {
      ...
      assert.ok(await name.includes(0), `${name} did not includes the offset`);
    });
    it("contains the fundraiser with the appropriate offset", async () => {
      const fundraisers = await factory.fundraisers(1, 0);
      const fundraiser = await FundraiserContract.at(fundraisers[0]);
  ...
  describe("boundary conditions", () => {
    ...
    it("raises out of bounds error", async () => {
      try {
        ...
        assert.ok(err.message.includes(expectedError), `${err.message}`);
      }
    });
